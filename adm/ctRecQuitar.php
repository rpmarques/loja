...
if ($_POST && isset($_POST['id'])) {
  ...
  $ret = $objContasReceber->quitar($id, $nronf, $serie, $valor, $ordem, $formaPgto, $datap);
}
?>
      ...
      <div class="row">
        ...
        <div class="col-md-12">
          ...
          if (isset($ret)) {
            if ($ret) {
              $ok = "true";
              require_once './alertaSucessoQuitacaoCtRec.php';
            } else {
          ...
          if ($ok == "false") { ?>
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Quitação de Conta - Nro:<?= $ctrec->nronf ?> - Série:<?= $ctrec->serie ?> - Parcela:<?= $ctrec->ordem . "/" . $ctrec->total_ordem ?></h3>
              </div> <!-- /.card-header -->
              <!-- form start -->
              <form method="post">
